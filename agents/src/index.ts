...
export const validator: AgentStage = {
  ...
  // report.html is regenerated by the pipeline once this stage's final
  // pass/fail status is persisted — not from inside runValidation itself,
  // so the report's own stage timeline shows validate's true outcome
  // rather than the transient 'running' status.
  regenerateReportAfter: true,
};
